    ...
    while (max_rounds >= current_round and
        ...
        for attacking_unit in attack_order:
            ...
            if battle_finished:
                break
        current_round += 1
        team_even_hp = sum(unit.current_vitality_points if unit.current_vitality_points > 0 else 0 for unit in team_even)
        team_odd_hp = sum(unit.current_vitality_points if unit.current_vitality_points > 0 else 0 for unit in team_odd)
